...
function renderRegister() {
    ...
    PlateStyles.forEach(s => {
        stylesHtml += `
            <div class="style-option ${s.id === SelectedStyle ? 'selected' : ''} style-${s.id}" onclick="selectStyle('${s.id}')">
                <div class="plate-preview">ABC-123</div>
                <strong>${s.label}</strong><br>
                <small>$${s.fee} fee</small>
            </div>
        `;
    });
    stylesHtml += '</div>';
...
window.addEventListener('message', (e) => {
    ...
    if (data.action === 'show') {
        ...
        resDiv.innerHTML = `
            <div class="card">
                <h3>Registration Details: ${info.plate}</h3>
                <p><strong>Owner ID:</strong> ${info.owner_identifier}</p>
                <p><strong>Status:</strong> <span class="status-badge status-${info.status}">${info.status}</span></p>
                <p><strong>Expiry Date:</strong> ${info.formatted_expiry}</p>
                <p><strong>Pink Slip:</strong> <span class="status-badge status-${info.pink_status}">${info.formatted_pink_expiry}</span></p>
                <p><strong>Plate Style:</strong> ${info.plate_style}</p>
                ${info.flag ? `<div class="card" style="border-color: #d32f2f; background: #fff0f0; margin-top: 15px;">
                    <p><strong>FLAGGED:</strong> ${info.flag.reason}</p>
                    <small>By ${info.flag.actor_identifier}</small>
                </div>` : ''}
            </div>
        `;
        ...
        if (!rows.length) html += '<p>No history found for this vehicle.</p>';
            ...
            <div class="history-item">
                <strong>${r.action.toUpperCase()}</strong> • $${r.fee}<br>
                <small>${r.created_at} • Actor: ${r.actor_identifier}</small>
            </div>
        `).join('');
